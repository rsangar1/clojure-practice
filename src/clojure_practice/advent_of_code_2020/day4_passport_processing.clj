...
#_(defn data->passport-details
  [data]
  (loop [v data
         p (first v)
         result []]
    (when (or (= (first v) "") (= (count (rest v)) 0))
      (conj result p))
    (recur (rest v)
           (if (= (first v) 0)
             ""
...
(comment
  ...
  (data->passport-details (input-file->data "resources/passport_processing_input1.txt"))

  (data->passport-details ["a" "" "b" "c" "" "d" "e" "f"])

  (split-with (complement #{""}) (input-file->data "resources/passport_processing_input1.txt"))
